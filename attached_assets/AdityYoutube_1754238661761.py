...
async def get_stream_url(query, video=False):
    # TubeAPI Configuration
    api_url = "http://localhost:5000/api"  # Change to your TubeAPI domain
    api_key = "sk-admin-test-key-10k-requests"  # Use your TubeAPI key
    
    # Extract video ID using VideosSearch (more reliable than URL parsing)
    try:
        if "&" in query:
            query = query.split("&")[0]
        ...
        if not search_result["result"]:
            print("No video found for query:", query)
            return ""
